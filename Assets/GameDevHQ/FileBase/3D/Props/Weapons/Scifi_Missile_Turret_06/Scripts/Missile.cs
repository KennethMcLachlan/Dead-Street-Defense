    ...
    public class Missile : MonoBehaviour
    {
        ...
        void FixedUpdate()
        {
            ...
            if (_thrust == true) //if thrust is true
            {
                ...
                _rigidbody.linearVelocity = transform.forward * _power; //set velocity multiplied by the power variable
                _thrust = false; //set thrust bool to false
            ...
            if (_trackRotation == true) //check track rotation bool
            {
                ...
                {
                    _rigidbody.rotation = Quaternion.LookRotation(_rigidbody.linearVelocity); // adjust rotation of rocket based on velocity
                ...
                else if (_missileType == Missile_Launcher.MissileType.Homing) //if missle is homing
                {
                    ...
                    _rigidbody.linearVelocity = transform.forward * _power; //apply forward velocity
        ...
        private void OnCollisionEnter(Collision other)           // Will have to adjust based on project needs
        {
