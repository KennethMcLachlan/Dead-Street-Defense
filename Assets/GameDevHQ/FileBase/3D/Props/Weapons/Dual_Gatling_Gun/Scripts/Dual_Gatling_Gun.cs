...
namespace GameDevHQ.FileBase.Dual_Gatling_Gun
{
    ...
    public class Dual_Gatling_Gun : MonoBehaviour
    {
        ...
        void Update()
        {
            if (Input.GetMouseButton(0)) //Check for left click (held) user input      || Rewrite for when enemy is active and nearby**************
            { 
                ...
                {
                    _audioSource.Play(); //play audio clip attached to audio source         ||       Add Audiosource clip
                    _startWeaponNoise = false; //set the start weapon noise value to false to prevent calling it again
                }

            }
            else if (Input.GetMouseButtonUp(0)) //Check for left click (release) user input            ||       Rewrite for when enemy is out of range *****************
            {
                //for loop to iterate through all muzzle flash objects
                for (int i = 0; i < _muzzleFlash.Length; i++)
                {
            ...
            //Add Look At Script to be active when enemy is in range *****************
        }
